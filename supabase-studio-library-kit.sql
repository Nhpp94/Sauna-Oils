-- Studio Library + Kit migration
-- Adds shared custom oils, blends, incense, and studio kit inventory.

-- Requires the RLS helper functions from supabase-fix-rls.sql:
...
-- Shared Studio custom oil definitions. Rows are full EssentialOil-compatible
-- records so older session JSON can keep rendering even if a definition changes.
